...
converter.toObject = function toObject(mtype) {
    ...
    ("if(m%s!=null&&m.hasOwnProperty(%j)){", prop, field.name); 
    ...
    for (; i <  mtype.fieldsArray.length; ++i) {
        ...
        if (field.map) { gen
            ...
            if (types.basic[type] === undefined) gen
                            ("value=types[%i].decode(r,r.uint32(),undefined,n+1)", i); 
            else gen
                            ("value=r.%s()", type);
...
function encoder(mtype) {
    ...
    ("if(%s!=null&&Object.hasOwnProperty.call(m,%j)){", ref, field.name) 
        ("for(var ks=Object.keys(%s),i=0;i<ks.length;++i){", ref)
            ...
            ("types[%i].encode(%s[ks[i]],w.uint32(18).fork()).ldelim().ldelim()", index, ref); 
            else gen
