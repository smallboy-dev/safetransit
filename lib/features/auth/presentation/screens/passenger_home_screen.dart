...
class _PassengerHomeScreenState extends State<PassengerHomeScreen> {
  ...
  String _calculateETA(Map<String, dynamic>? location) {
    if (location == null || location['latitude'] == null || location['longitude'] == null) {
      return '5 min'; 
    }

    final double driverLat = (location['latitude'] as num).toDouble();
    final double driverLng = (location['longitude'] as num).toDouble();

    final double latDist = (driverLat - _passengerLat).abs() * 111;
    final double lngDist = (driverLng - _passengerLng).abs() * 111 * math.cos(_passengerLat * math.pi / 180);
    
    final double distance = math.sqrt(latDist * latDist + lngDist * lngDist);

    final int minutes = (distance / 0.5).round();
            ...
            children: [
              ...
              if (drivers.isEmpty)
                ...
                ...drivers.map((doc) {
                  ...
                  final rating = isSimSwapped ? 2.1 : 5.0;
                  final eta = _calculateETA(data['location']);
  ...
  Widget _buildRouteCard(String route, String type, String eta, {String? rating}) {
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              isBus ? LucideIcons.bus : LucideIcons.car,
              color: AppTheme.primaryColor,
              size: 24,
            ),
          ),
